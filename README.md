...
- **Stage 2 parser coverage** (parserTests, 1077 cases): **27.1%** parse with **zero diagnostics** — fully inside the engine's declared support range (paragraphs, headings, bold/italic, internal + external links, flat lists). Honest *coverage*, not correctness; it climbs as the supported subset grows. Track: `cargo test --test parser_tests stage2_coverage_rate`.
