...
- **Sample-article throughput** (criterion, `benches/compare.rs`):
  ...
  > The Stage 2 **AST path** (parse → plain text) is ~1.3× faster than the Stage 1 `strip` while producing both text **and** diagnostics. It **does not expand templates** — it drops them with a `W-TEMPLATE` warning and keeps the surrounding prose. Expanding templates (à la Bliki) would mean a Lua/Scribunto engine and ~2 orders of magnitude slower (Bliki runs at ~0.4 MB/s) — surrendering the one advantage wikrs has. So: honest drop + flag, keep the speed.
...
- **Stage 2 differential — the "three numbers"** (layer 2 of [docs/TESTING.md](docs/TESTING.md); the headline Stage-2 DoD): wikrs's extracted prose vs **Parsoid's** rendered HTML over a fixed, committed sample of real pages. Seed run (**18 featured-class articles**, fetched 2026-06-27):
    - **~93% mean precision** — of the prose wikrs emits, ~93% is corroborated by the article (**17/18 pages fully faithful**). A *conservative* floor: the residual gap is `<math>` LaTeX / `<ref>` markup and table-cell reordering, not silent garbling — the spread is tight with **zero precision outliers**.
    ...
    Reproduce: `cargo xtask diff-fetch && cargo xtask diff-report` (pages cached gitignored; only the names-only title lists are committed). **Representative evidence** — on **120 random ns0 pages** (`cargo xtask diff-sample`): mean precision **~89%**, **68% of pages fully faithful**, and a **9.2% silent structural-diff** rate (down from 40% before the `[[File:…]]`/`[[Category:…]]` link leaks were fixed). The residual silent cases are mostly `<ref>` markup leaking on list pages (the next target); the lowest-precision pages overall are sports/stats *tables* wikrs **honestly flags** (Reported), not silent errors. A real sample now — not yet N-thousand, but evidence.
