...
- **Stage 2 parser coverage** (parserTests, 1077 cases): **36.8%** parse with **zero diagnostics** — fully inside the engine's declared support range (paragraphs, headings, bold/italic, internal + external links, flat & definition lists, refs/nowiki/comments, inline HTML formatting tags). Honest *coverage*, not correctness; it climbs as the supported subset grows (templates are the big remaining blocker). Track: `cargo test --test parser_tests stage2_coverage_rate`.
