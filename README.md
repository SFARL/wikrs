...
`--format text` (default) emits one article's plain text per record; `--format jsonl` emits `{"title":…,"text":…,"diagnostics":[…]}` per line; `--format sections` emits one JSON object per page with the article split into flat, level-tagged sections — `{"title", "sections": [{"level", "heading", "text"}], "diagnostics"}` — ready for RAG chunking (`level` = the heading's `=` count, lead section is `level: 0`); `--format markdown` emits GFM markdown per page (`# title` + structure-preserving body: headings, links, lists, tables; out-of-range constructs appear as visible ` ```wikitext ` fenced blocks). Markdown output is **conformance-tested by construction**: a round-trip harness asserts that an independent GFM parser (pulldown-cmark) reads every emitted document back to exactly the structure the AST declares — over all 1,071 MediaWiki parserTests inputs and a fuzzer. Both `.xml` and multistream `.xml.bz2` are accepted. The default **`ast`** engine (Stage 2 parser: structured, honest diagnostics) handles real articles; pass `--engine strip` for the Stage 1 fast/lossy path.

**The diagnostics travel with the output.** Each `diagnostics` entry is `{"code","severity","start","end","message"}` (byte span into the page's wikitext); with `--engine strip` the key is absent entirely — Stage 1 *can't* diagnose, which is different from "checked and found nothing". `--stats` reports the parser's view next to the residual-markup heuristic (`zero-diag= warned= unsupported=`), plain `--format text` prints a one-line stderr summary when anything was flagged (stdout stays pure article text), and `--fail-on unsupported` (or the stricter `warning`) turns any flagged page into a non-zero exit code for pipelines. 
