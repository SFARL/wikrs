...
- **Stage 2 differential — the "three numbers"** (layer 2 of [docs/TESTING.md](docs/TESTING.md); the headline Stage-2 DoD): wikrs's extracted prose vs **Parsoid's** rendered HTML over a fixed, committed sample of real pages. Seed run (**18 featured-class articles**, fetched 2026-06-27):
    ...
    Reproduce: `cargo xtask diff-fetch && cargo xtask diff-report` (pages cached gitignored; only the names-only title lists are committed). **Representative check** — on 25 *random* ns0 pages (`cargo xtask diff-sample`), precision is **~92%** and the **silent structural-diff rate is just 4%** (1/25) — down from **40%** before the differential caught `[[File:…]]` / `[[Category:…]]` link markup leaking into output (no diagnostic → the silent-error driver on short stubs). Still small samples — a methodology proof, not yet the N-thousand-page run.
