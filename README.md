...
- **Conversion rate (residual-markup floor):** on the **full real simplewiki dump** (281,799 articles, default AST engine) **91.9% of pages convert clean** — no leaked `{{`/`[[`/`{|` markup in the output (`wikrs --input dump.xml.bz2 --stats`). On the 1,077 synthetic parserTests cases it's **98.1%** (`cargo test --test parser_tests stage1_conversion_rate`); real articles are messier, hence the gap. This is a *leniency floor* — it catches markup that **leaked**, not correctness-vs-Parsoid (that's the Stage 2 differential below). The ~8% leak tail is dominated by stray **`]]`** (6.7% of pages — File/image-caption tails with a nested `[[wikilink]]`, where the drop matches the inner `]]`) and **`|}`** (1.3%); all else <0.3%.
