...
- **Stage 2 parser coverage** (parserTests, 1077 cases): **24.8%** parse with **zero diagnostics** — fully inside the engine's declared support range (paragraphs, headings, bold/italic, internal links). Honest *coverage*, not correctness; it climbs as the supported subset grows. Track: `cargo test --test parser_tests stage2_coverage_rate`.
