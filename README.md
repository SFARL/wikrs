...
The CLI **streams** the dump in bounded batches — constant memory regardless of dump size (~96 MB peak on a 1.7 GB dump; **210 MB on the full 26.4 GB enwiki**) — and a dump read/decode error is a **hard error with a real exit code**, never a silently skipped page. The full English Wikipedia (`pages-articles-multistream.xml.bz2`, 7.19M articles) runs end-to-end in **~38 minutes on an Apple-Silicon laptop**, single stream; the wall time is bz2-decompression-bound (parallel multistream decoding is the known next speed step).
