...
|---|---|---|
| `Text` | `Run{text, bold, italic}` | 实体解码；空白串折叠为单空格；**NUL→U+FFFD**（CommonMark 输入归一化，任何消费者都会做）；同格式相邻 Run **先合并**（pulldown 在实体处拆 Text 事件）；块首尾去空白；空 Run 丢弃 |
| `Bold`/`Italic` | Run 上的布尔标志（**拍平，不保留嵌套顺序**） | `Bold(Italic(x))` ≡ `Italic(Bold(x))`；**样式 run 边缘的非字母数字字符剥成普通 Run**（CommonMark 侧翼规则使空格/标点收边的 emphasis 不可解析——`* and *`、`*hot!*x`；剥后字母数字收边，定界符边界恒有效；无字母数字核心的 run 整体失去样式） |
| `Link` | `Link{href, label:Vec<Run>}` | 内链 href 过 `md_href`（实体先解码=MediaWiki 标题语义、空格→`_`、RFC3986 路径集**除 `&`**、其余 %XX、`./` 前缀）；外链过滤:控制字节/`<>`/`|`/`\` %XX、"形如实体"的 `&`（`&#…;`/`&name;`）→`%26`（防 MD 目标二次实体解码,裸查询分隔符 `&a=1` 保留）；空 label（裸外链）→ label = href 文本；**label 内嵌套链接拍平成文本**（MD 不能嵌链接）；**空 target 链接拍平成 label 文本**（`[[]]` 废字面量） |
| `Heading{level}` | `Heading(level.clamp(1,6), inlines)` | level=等号数直映（`==`→2→`##`） |
| `Paragraph` | `Para(inlines)` | 归一化后空 → 整块丢弃 |
| `List{ordered,items}` | `List{ordered, items:[{content, sublists}]}` | 有序列表起始编号不比（恒发 `1.`）；紧/松列表不比（item 内段落解包）；**无内容且无子列表的 item 丢弃、零 item 列表丢弃**（CommonMark 禁空项打断段落,空 wikitext 项无语义） |
| `Preformatted` | `Code{info:"", text}` | **行内格式拍平成纯文本**（fenced block 只能装字面文本——声明损失）；尾空白去除；NUL→U+FFFD；**CR/CRLF→LF**（CommonMark 把 `\r` 当行结束,无法逐字往返） |
| `Unsupported` | `Code{info:"wikitext", text:源码逐字（行尾归一）}` | M3 可见标记：fence 长度=内容最长反引号串+1（≥3），info 串 `wikitext` 可 CSS/grep 定位 |
| `Table{rows}` | `Table{rows}`（cell=Vec<Run/Link>） | GFM 强制**矩形**：行按最大宽度补空 cell 比对（表头=row0 定列数）；NF 把 pulldown 的 head+body 拼回一个 rows；对齐说明符不比；空表/**零列表格**（`{\|`+空行）丢弃 |

**渲染器侧的防走火（不改 NF,只保证发出的文本解析回 NF）:** `|` 全语境转义（列表项与子列表行连续,`x|`+`- |` 会凭空拼出 GFM 表格）；标题内 `#` 全转义（行尾 `#` 串=ATX 闭合序列）；Link 前裸 `!` 回退转义（防拼成图片语法）；同类型相邻列表交替 bullet（`-`/`*`、`1.`/`1)`,单空行分隔的同类列表会被合并）；父项有文本且首个子列表第一项为空时先发空行（空项列表不能打断段落）；行首 `# > - + = ~` 与 `\d+[.)]` 转义。
